...
struct ssd_info *initiation(struct ssd_info *ssd)
{
	...
	ssd->gapoutputfile=fopen(ssd->gapfilename,"w");
	if(ssd->gapoutputfile==NULL)
	{
		printf("the gap output file can't open\n");
	}
...
struct page_info * initialize_page(struct page_info * p_page,int random  )
{
	...
	p_page->pv = (unsigned int )(random%7+1);
